...
export async function patchBoardGameTrackerUpdateScoreCard(
    {documentId, token, game, date, isCompleted, location, playerScores}:
  ...
  if (date !== undefined) {
    anyModified = true
    payload['date'] =
        date  // TODO: unify date formats on client (API is using YYYY-MM-DD)
  }
  ...
  if (!anyModified) {
    return {
      success: false, error: 'no fields modified'
    }
  }
  ...
  try {
    const response = await fetch(
        `${API_BASE_URL}/update-score-card/${documentId}`,
